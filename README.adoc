...
= Hardening Ubuntu and Debian. Systemd edition.

:icons: font

A quick way to make a Ubuntu or Debian server a bit more secure.
...
Tested on `Ubuntu 20.04 Focal Fossa`, `Ubuntu 22.04 Jammy Jellyfish`,
`Ubuntu 24.04 Noble Numbat`, and `Debian`.
...
. Perform a minimal Debian installation.
. Partition the system, see below for recommendations.
. Do not install a desktop environment or any additional services.
. Finish the installation and reboot.
. Log in.
...
Sets `apt` flags, performs basic permission check, and validates
that the system is running Ubuntu or Debian.
...
`postfix`, `psad`, `rkhunter`, `sysstat`, `systemd-coredump`, `tcpd`, `vlock`.

On Ubuntu, `update-notifier-common` is also installed.
...
On Ubuntu, also disables
https://manpages.ubuntu.com/manpages/jammy/man1/apport-cli.1.html[apport] and
https://github.com/Ubuntu/ubuntu-report[ubuntu-report].

The `apport` function is located in link:scripts/apport[./scripts/apport].

==== `motdnews`

Disables https://ubuntu.com/legal/motd[motd-news].
On Ubuntu, also disables `apt_news` via `pro`.
...
Removes the `autofs`, `avahi*`, `beep`, `git`, `pastebinit`,
`popularity-contest`, `rsh*`, `rsync`, `talk*`, `telnet*`, `tftp*`,
`xinetd`, `yp-tools`, `ypbind` packages.

On Ubuntu, also removes `apport*` and `whoopsie`.
...
On Debian, `/snap/bin` is omitted from the PATH.
...
is installed and GRUB is updated.

On Ubuntu, https://packages.ubuntu.com/jammy/secureboot-db[secureboot-db] is also installed.
